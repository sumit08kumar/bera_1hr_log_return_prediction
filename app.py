...
@app.route(config.API["ENDPOINTS"]["FORECAST"], methods=['POST'])
def forecast():
    try:
...
def get_inference(token):
    try:
        logger.info(f"Generating inference for token: {token}")
        if predictor is None or predictor.model is None:
            return Response("Model not trained yet", status=400)
        if predictor.X_test is None:
            return Response("No test data available; train the model first", status=400)
        test_features = predictor.X_test[-1].reshape(1, -1)
        prediction = predictor.predict(test_features)
        current_price = predictor.df['close'].iloc[-1]
        predicted_price = current_price * np.exp(prediction[0])
        logger.info(f"Inference generated successfully for {token}")
        return Response(f"Predicted Price: {predicted_price}", status=200)
    except Exception as e:
        logger.error(f"Inference failed for {token}: {str(e)}")
        return Response(f"Failed to generate inference: {str(e)}", status=500)
